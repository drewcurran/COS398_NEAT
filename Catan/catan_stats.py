'''
catan_players.py
Description: Module to show iteration stats.
Adaptor: Drew Curran
'''
...
def play_batch_core(num_games, players, game_config, accumulators=[], population_players=None):
...
def play_batch(
    ...
    if population_players is not None:
        population_size = len(population_players)
    else:
        population_size = 1
    ...
    if quiet:
        for _ in play_batch_core(num_games, players, game_config, accumulators, population_players=population_players):
            pass
        return (
    ...
    # ===== Game Details
    last_n = 10
    actual_last_n = min(last_n, num_games * population_size)
    ...
    with Progress(
        ...
        main_task = progress.add_task(f"Playing {population_size} x {num_games} games...", total=num_games * population_size)
        player_tasks = [
            progress.add_task(
                rich_player_name(player), total=num_games * population_size, show_time=False
            )
            for player in players
        ]

        for i, game in enumerate(
            play_batch_core(num_games, players, game_config, accumulators, population_players=population_players)
        ):
            winning_color = game.winning_color()

            if (num_games * population_size - last_n) < (i + 1):
                seating = ",".join([rich_color(c) for c in game.state.colors])
                row = [
                    "%s x %s" % (str(i // num_games + 1).rjust(4), str(i % num_games + 1).ljust(4)),
                    seating,
